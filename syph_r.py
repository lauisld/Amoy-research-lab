...
# Matches a read to a specified string of nucleotides, "primer", with less than n errors. # Modified by Elaine
def fuzzy_match(read_seq, primer, n):
	...
	# If can't find exact match, searches for best match with less than 3 substitutions.
		...
		fuzzy_match = regex.search(r"(?b)("+primer + "){s<="+str(n)+"}", read_seq)
		if fuzzy_match:
			return fuzzy_match[0]


# String matches a read to a variable region (V1-V7). # Modified by Elaine
def region_seq(read_seq, read_name, sample_name, is_pacbio, current_dir):
	...
	for region, primer in variable_regions.items():
		...
		match_seq = fuzzy_match(read_seq, primer, '2')
		match_seq_rev = fuzzy_match(read_seq_rev, primer, '2')
		...
		if match_seq or match_seq_rev:
			...
			if not match_seq and match_seq_rev:
				read_seq = read_seq_rev
				match_seq = match_seq_rev
			if (region == "V1"):
				start = (str.index(read_seq, match_seq) + 12)
				if fuzzy_match(read_seq, "ATGCCCCAGGCCAGCT", '2'):
					end = (str.index(read_seq, fuzzy_match(read_seq, "GCCCCAGGCCAGCT", '2')))
					v_list = V1_list
					v_dna = V1_dna
			elif (region == "V2"):
				start = (str.index(read_seq, match_seq) + len(match_seq))
				if fuzzy_match(read_seq, "CCCAGTCGGTGTTAGA", '2'):
					end = (str.index(read_seq, fuzzy_match(read_seq, "GTCGGTGTTAGA", '2')))
					v_list = V2_list
					v_dna = V2_dna
				elif fuzzy_match_v2(read_seq, "CCCAGTCGGTGTTAGA"):
					end = (str.index(read_seq, fuzzy_match(read_seq, "GTCGGTGTTAGA", '5')))
					v_list = V2_list
					v_dna = V2_dna
			elif (region == "V3"):
				start = (str.index(read_seq, match_seq) + 12)
				if fuzzy_match(read_seq, "GTCGCTAGACAG", '2'):
					end = (str.index(read_seq, fuzzy_match(read_seq, "GTCGCTAGACAG", '2')))
					v_list = V3_list
					v_dna = V3_dna
				elif fuzzy_match(read_seq, "GTCGCTAGACAG", '3'):
					end = (str.index(read_seq, fuzzy_match_v3(read_seq, "GTCGCTAGACAG", '3')))
					v_list = V3_list
					v_dna = V3_dna
			elif (region == "V4"):
				start = (str.index(read_seq, match_seq) + 12)
				if fuzzy_match(read_seq, "TCTGTAGCAGCCAGAG", '2'):
					end = (str.index(read_seq, fuzzy_match(read_seq, "TGTAGCAGCCAGAG", '2')))
					v_list = V4_list
					v_dna = V4_dna
			elif (region == "V5"):
				...
				if fuzzy_match(read_seq, "AATATCCTCCCGCCGA", '2'):
					end = (str.index(read_seq, fuzzy_match(read_seq, "TCCTCCCGCCGA", '2')))
					v_list = V5_list
					v_dna = V5_dna
			elif (region == "V6"):
				start = (str.index(read_seq, match_seq) + 12)
				if fuzzy_match(read_seq, "GGCATGTACGTACG", '2'):
					end = (str.index(read_seq, fuzzy_match(read_seq, "CATGTACGTACG", '2')))
					v_list = V6_list
					v_dna = V6_dna
			elif (region == "V7"):
				start = (str.index(read_seq, match_seq) + 16)
				if fuzzy_match(read_seq, "CGTACAAGTTTGCATA", '2'):
					end = (str.index(read_seq, fuzzy_match(read_seq, "CAAGTTTGCATA", '2')))
					v_list = V7_list
					v_dna = V7_dna
			if end != 0:
				...
				region_nuc = str((Seq(region_seq, generic_dna).reverse_complement())).strip()
				region_seq = translate_nucs(region_nuc)
				all_assignments.write(sample_name + "," + read_name.rstrip()  + "," + region_seq + "," + 
					region_nuc + "," + region + "," + str(start) + "," + str(end) + "\n")
