...
julia <- julia_setup(JULIA_HOME = "/Projects/tprk/softwares/julia-9d11f62bcb/bin/") # Modified by Elaine
...
metadata <- read.table(paste(path,"/metadata.csv", sep=''), sep=',', header=TRUE, check.names=FALSE) # Modified by Elaine
setwd(path)
...
tprKF <- "CCATACATCCCTACCAAATCACG" # Modified by Elaine
tprKR <- "CGCAGTTCCGGATTCTGA" # Modified by Elaine
rc <- dada2:::rc
...
if(opt$illumina == FALSE) {
  ...
  mkdir_freq <- paste("mkdir -p ",PacBio_freq_path,sep='') # Modified by Elaine
  ...
  for (num in c(1:length(PacBio_freq_path))) {
    syphrPacBio_command <- paste("python3 ",syph_path,
                                 " -i fasta -pacbio -d ",PacBio_freq_path[num],"/",sep='') # Modified by Elaine
...
if (opt$pacbio == FALSE) {
  ...
  syphrIllumina_command <- paste("python3 ",syph_path," -i fastq -illumina -d ",Illumina_freq_path,"/ ;",sep='') # Modified by Elaine
